...
function someEven () {
    return numbers.reduce((contador, numero) => (numero % 2 === 0) ? contador += numero : contador)
}
...
function melhorMateria () {
    return estudantes.map((estudante) => ({name: estudante.nome, materia: estudante.materias.reduce( (maior, numero) => (maior.nota > numero.nota) ? maior : numero).name}));
}
