...
-- Monte uma query que exiba o id do ator, nome do ator e id do 
-- filme em que ele já atuou, usando as tabelas actor e film_actor.
SELECT f.film_id, a.actor_id, a.first_name 
...
-- ordenados pelo nome em ordem decrescente, juntamente com o id do endereço e o 
-- nome da rua onde o cliente mora. Essas informações podem ser encontradas nas tabelas customer e address.
...
SELECT COUNT(a.address_id), c.first_name
FROM customer AS c
INNER JOIN address AS a ON a.address_id = c.address_id
WHERE c.active = 1
...
-- paga aos funcionários no ano de 2006. Use as tabelas payment e staff. Os resultados devem estar agrupados pelo nome e sobrenome do funcionário.
SELECT CONCAT(s.first_name, ' ', s.last_name) AS nome, AVG(p.amount)
FROM staff AS s
...
SELECT a.actor_id, a.first_name, b.film_id, b.title 
FROM film_actor AS f
INNER JOIN actor AS a ON f.actor_id = a.actor_id
INNER JOIN film AS b ON f.film_id = b.film_id;
